...
# Vortrainiertes ResNet50 Modell für EuroSAT laden
model = models.resnet50(weights=None)
model.fc = torch.nn.Linear(model.fc.in_features, len(EUROSAT_CLASSES))
model.load_state_dict(torch.load("eurosat_resnet50.pth", map_location=torch.device('cpu')))
model.eval()
...
<body>
  ...
  <script>
    ...
    if (!imageInput.files[0]) {
        ...
        feedbackResultDiv.classList.add('d-none');
        return;
    }
    ...
    .then(data => {
        ...
        feedbackResultDiv.classList.add('d-none'); // Feedback-Resultat zurücksetzen
        imageInput.value = ''; // Eingabefeld zurücksetzen
    })
    .catch(error => {
...
def classify():
    ...
    try:
        ...
        return {"error": "Ungültiges Bildformat."}, 400
    ...
    with torch.no_grad():
        ...
        class_name = EUROSAT_CLASSES[predicted.item()]
